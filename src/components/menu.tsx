...
export default function MenuSection() {
  ...
  <h1 className="text-4xl font-bold text-center">
    From <span className="text-amber-500">Our Menu</span>
      ...
      {/* Navigation Menu */}
      <nav className="flex flex-wrap md:justify-center justify-start mt-10 gap-6 text-lg">
       {["Breakfast", "Lunch", "Dinner", "Dessert", "Drink", "Snack", "Soups"].map((item, index) => (
      <span
      key={index}
      className={`cursor-pointer ${
        item === "Breakfast" ? "text-amber-500 font-bold" : "text-white"
      }`}
      >
      {item}
       </span>
  ...
  width={350}
  height={100}
  className="absolute bottom-10 right-28 rounded-full border-4 border-white z-10"
/>
